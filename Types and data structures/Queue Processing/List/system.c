...
void insert_first_request(system_struct* system)
{
    ...
    void* temp =(void*)insert_node(&(system->first_queue), create_first_request());
...
void insert_second_request(system_struct* system)
{
    ...
    void* temp = (void*)insert_node(&(system->second_queue), create_second_request());
    if (system->need_print_adress) {
        printf("added- %p\n", temp);
    }
    return;
...
request_struct* get_request_from_ques(system_struct *system) {
    ...
    if (system->first_queue == NULL )
    {
        if (system->second_queue != NULL) {
            if (system->need_print_adress) {
                printf("deleted- %p\n", (void*)system->second_queue);
            }
            system->len_sum_second += count(system->second_queue);
            system->len_count_sum_second += 1;
...
void tick(system_struct *system)
{
    ...
    node_struct *p = system->first_queue;
    while (p != NULL) {
        p->value->counter =  p->value->counter + 1;
        p = p->next;
    }

    p = system->second_queue;
    while (p != NULL) {
        p->value->counter =  p->value->counter + 1;
        p = p->next;
    }
